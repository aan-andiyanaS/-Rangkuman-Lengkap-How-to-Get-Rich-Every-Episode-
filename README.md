...
```mermaid
mindmap
  root((HOW TO GET RICH))
    Uang & Kekayaan
      Wealth vs Money vs Status
      Pasar Bebas
      Bunga Majemuk
      Kebebasan = Pensiun Sejati
    Kewirausahaan
      Productize Yourself
      Founder-Product-Market Fit
      Autentisitas vs Kompetisi
      Sprint-Inspirasi-Istirahat
    Leverage
      Labor (buruk, perlu izin)
      Capital (kuat, perlu izin)
      Code & Media (terbaik, permissionless)
      Internet sebagai Skala Global
    Skill Penting
      Belajar Menjual
      Belajar Membangun
      Membaca & Belajar
...
```mermaid
flowchart LR
    A[🧬 Sifat Kooperatif Manusia] --> B[Pertukaran & Perdagangan]
    B --> C[Pasar Bebas]
    C --> D[Penciptaan Kekayaan]
    D --> E[Kemakmuran untuk Semua]
    E --> F[Positive-Sum Game]
    F -->|Siklus berlanjut| B
```
...
```mermaid
graph TD
    subgraph "❌ Model Lama: Jual Waktu"
        A1[Kerja 8 jam] --> B1[Dapat gaji harian]
        B1 --> C1[Kerja lagi esok hari]
        C1 --> D1[Plateau — tidak pernah skala]
    end

    subgraph "✅ Model Baru: Leverage & Aset"
        A2[Bangun Aset / Produk / Kode / Media] --> B2[Aset bekerja 24/7]
        B2 --> C2[Penghasilan saat tidur]
        C2 --> D2[Skala eksponensial]
    end
```
...
```mermaid
flowchart LR
    A[Penghasilan naik] --> B[Upgrade gaya hidup]
    B --> C[Pengeluaran naik seiring penghasilan]
    C --> D[Tabungan tetap sama]
    D --> E[Tidak pernah bebas secara finansial]
    E --> F[Kerja lebih keras untuk mempertahankan gaya hidup]
    F --> A
    style E fill:#ff6b6b,color:#fff
    style A fill:#74c69d,color:#fff
```
...
```mermaid
graph TD
    F[🧑 FOUNDER<br/>Siapa kamu?<br/>Apa obsesimu?<br/>Di mana kamu natural?]
    P[📦 PRODUCT<br/>Apa yang kamu bangun?<br/>Apakah kamu sendiri butuh ini?<br/>Bisakah direplikasi tanpa biaya?]
    M[🏪 MARKET<br/>Siapa yang mau membayar?<br/>Apakah pasarnya berkembang?<br/>Adakah efek jaringan?]

    F <-->|"Autentisitas"| P
    P <-->|"Skalabilitas"| M
    M <-->|"Relevansi"| F

    FIT["🎯 FOUNDER-PRODUCT-MARKET FIT<br/>= Zona Tak Terkalahkan"]
    F & P & M --> FIT
    style FIT fill:#2d6a4f,color:#fff,font-weight:bold
```
...
```mermaid
flowchart LR
    I[💡 INSPIRASI<br/>Muncul tiba-tiba<br/>Harus segera ditangkap] --> S[🏃 SPRINT<br/>Kerja sekeras mungkin<br/>saat termotivasi]
    S --> R[😴 ISTIRAHAT<br/>Pulihkan energi<br/>Evaluasi ulang]
    R --> A[📊 ASESMEN<br/>Nilai apa yang sudah dicapai<br/>Rencanakan sprint berikutnya]
    A --> I
...
graph TD
    ...
    subgraph L1["👷 Labor (Tenaga Kerja)"]
        direction TB
        la[Orang bekerja untukmu]
        lb[Perlu izin — harus bisa memimpin]
        lc[TERBURUK dari semua bentuk leverage]
        ld[Tinggi biaya koordinasi & manajemen]
        la --> lb --> lc --> ld
    end

    subgraph L2["💰 Capital (Modal)"]
        direction TB
        ca[Uang bekerja untukmu]
        cb[Perlu izin — harus dicari/dipercayakan]
        cc[Sangat powerful tapi selektif]
        cd[Butuh track record & reputasi]
        ca --> cb --> cc --> cd
    end

    subgraph L3["💻 Code & Media"]
        direction TB
        ba[Kode / Konten bekerja untukmu 24/7]
        bb[🔓 PERMISSIONLESS — tidak butuh izin siapapun]
        bc[TERBAIK: biaya replikasi nol]
        bd[Satu blog, satu video, satu aplikasi → jutaan orang]
        ba --> bb --> bc --> bd
    end

    style lc fill:#ff6b6b,color:#fff
    style bb fill:#52b788,color:#fff
    style bc fill:#52b788,color:#fff
```
...
```mermaid
flowchart TD
    A[Kamu + Minat Spesifik] --> B[Internet]
    B --> C[Menemukan Audiens Global]
    C --> D[Minat Niche Bisa Diskala]
    D --> E[Setiap orang adalah terbaik di sesuatu = menjadi diri sendiri]
    E --> F[Bisnis + Kekayaan + Dampak]
...
```mermaid
flowchart LR
    subgraph "Bukan Specific Knowledge"
        direction TB
        G1[Bisa dilatih di sekolah]
        G2[Bisa dioutsourcing]
        G3[Bisa diautomasikan]
    end

    subgraph "✅ Specific Knowledge"
        direction TB
        S1[Tidak bisa diajarkan]
        S2[Dibangun dari obsesi & minat tulus]
        S3[Sering sangat teknis ATAU sangat kreatif]
        S4[Terasa seperti bermain bagimu, tapi seperti kerja bagi orang lain]
    end

    SK[🧠 Specific Knowledge] --> S1 & S2 & S3 & S4
```
...
```mermaid
graph LR
    SELL["🗣️ BELAJAR MENJUAL<br/>(Marketing, Sales, Komunikasi,<br/>Membangun Kepercayaan)"]
    BUILD["⚙️ BELAJAR MEMBANGUN<br/>(Engineering, Desain, Coding,<br/>Manufaktur)"]
    BOTH["🚀 KOMBINASI KEDUANYA<br/>= Unstoppable<br/>Contoh: Elon Musk, Steve Jobs"]

    SELL -->|"Distribusi + Produk"| BOTH
    BUILD --> BOTH

    style BOTH fill:#e9c46a,color:#000,font-weight:bold
```
...
```mermaid
flowchart TD
    D1[Masalah / Keputusan] --> D2{Apakah reversible?}
    D2 -->|Ya - mudah dibatalkan| D3[Putuskan Cepat<br/>Bias ke Tindakan]
    D2 -->|Tidak - irreversible| D4[Pikirkan Mendalam<br/>Gunakan First Principles]
    D4 --> D5{Apa konsekuensi<br/>jangka panjangnya?}
    D5 --> D6[Pilih yang maximize<br/>opsi di masa depan]
    D3 & D6 --> D7[✅ Tindakan]
    D7 --> D8[Belajar dari Hasil]
    D8 -->|Feedback loop| D1

    style D3 fill:#52b788,color:#fff
    style D6 fill:#0077b6,color:#fff
```
...
```mermaid
graph TD
    subgraph "Tidak Bisa Dibeli, Harus Dibangun"
        H["💪 KESEHATAN FISIK<br/>(Fit Body)<br/>Harus dirawat setiap hari"]
        M["🧘 KETENANGAN PIKIRAN<br/>(Calm Mind)<br/>Melalui meditasi & praktik"]
        L["❤️ CINTA DALAM RUMAH<br/>(House Full of Love)<br/>Melalui hubungan yang dirawat"]
    end

    W[💰 Kekayaan Material] -.->|"Tidak bisa membeli ini"| H
    W -.-> M
    W -.-> L
    
    H & M & L --> REAL["🌟 KEKAYAAN SEJATI"]

    style REAL fill:#264653,color:#fff,font-weight:bold
...
```mermaid
flowchart TD
    A[🎯 TUJUAN: Kebebasan & Kekayaan] --> B

    subgraph B["FONDASI"]
        B1[Specific Knowledge<br/>Pengetahuan unik yang tidak bisa digantikan]
        B2[Akuntabilitas<br/>Ambil risiko atas nama sendiri]
        B3[Skill: Jual + Bangun<br/>Dua keterampilan paling powerful]
    end

    B --> C

    subgraph C["LEVERAGE"]
        C1[Code & Media<br/>Permissionless leverage terbaik]
        C2[Capital<br/>Setelah membangun reputasi]
        C3[Labor<br/>Minimum yang diperlukan]
    end

    C --> D

    subgraph D["MULTIPLIER"]
        D1[Judgment<br/>Keputusan yang benar]
        D2[Internet Scale<br/>Distribusi ke seluruh dunia]
        D3[Compound Time<br/>Bertahun-tahun konsistensi]
    end

    D --> E["💰 KEKAYAAN YANG BERKELANJUTAN"]
    E --> F["🔓 KEBEBASAN SEJATI"]
    F --> G["🌟 KEHIDUPAN YANG AUTENTIK & BERMAKNA"]
